...
class DriverStationGUI:
    ...
    def load_all_widgets(self):
        self.loopback_button = Button(self.win, text="Loopback Request", command=self.request_loopback).grid(row=7,
                                                                                                             column=7)
        ...
        self.h_lower_scale = Scale(self.win, from_=0, to=100, orient=HORIZONTAL, command=self.h_lower_readout).grid(
            row=1, column=1)
    ...
    def on_choices(self, choices):
        ...
        for c in choices:
            Radiobutton(self.win, text=c, variable=self.auto_number, value=i, command=self.auto_num).grid(row=i,
                                                                                                          column=0)
            i += 1
