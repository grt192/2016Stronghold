...
class NoLowMacro(GRTMacro):
	...
	def stop_mechs_start_drive(self):
		if self.timers_running:
			...
			threading.Timer(4.60, self.stop_drive_start_angle_change).start()

	def stop_drive_start_angle_change(self):
		if self.timers_running:
			self.straight_macro.disable()
			self.pickup.angle_change(.5)
			threading.Timer(1.8, self.stop_turn_start_shot).start()
