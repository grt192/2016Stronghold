...
class ChevalMacro(GRTMacro):
    ...
    def stop_mechs_start_drive(self):
        if self.timers_running:
            ...
            threading.Timer(1.2, self.pickup_down).start()

    def pickup_down(self):
        if self.timers_running:
            self.pickup.disable_automatic_control()
            self.pickup.angle_change(.5)
            self.straight_macro.disable()
            threading.Timer(1.80, self.stop_mechs_start_driveback).start()


    def stop_mechs_start_drive2(self):
        if self.timers_running:
            self.pickup.angle_change(0)
            self.straight_macro.POWER = -.85
            self.straight_macro.set_forward()
            self.straight_macro.enable()
            threading.Timer(1.5, self.stop_mechs_start_driveback2).start()
    ...
    def stop_mechs_start_driveback(self):
        if self.timers_running:
            ...
            self.straight_macro.enable()
            #threading.Timer()
            threading.Timer(.2, self.stop_mechs_start_drive2).start()

    def stop_mechs_start_driveback2(self):
        if self.timers_running:
            self.pickup.angle_change(0)
            self.straight_macro.POWER = -.4
            self.straight_macro.set_reverse()
            self.straight_macro.enable()
            #threading.Timer()
            threading.Timer(.4, self.stop_turn_start_shot).start()
