...
class Shooter:
    ...
    def execute_shot(self):
        ...
        self.turntable_sensor.on_target_count = 0
        threading.Timer(1.0, self.finish_automatic_shot).start()
    ...
    def vt_reverse_func(self):
        ...
        threading.Timer(0.4, self.vt_delay_func).start()
    ...
    def geo_reverse_func(self):
        self.flywheel.spin_to_reverse_power()
        threading.Timer(0.6, self.geo_delay_func).start()
    ...
    """
    Low goal shot initialization.
    """
    def low_automatic_shot(self):
        if self.rails.current_position == "up":
            self.rails.rails_down()
            threading.Timer(.5, self.execute_low_shot).start()
        else:
            self.execute_low_shot()

    def execute_low_shot(self):
        self.shooter_timers_running = True
        #self.drivecontroller.disable_manual_control()
        self.rails.rails_up()
        #self.dt.set_dt_output(0, 0)

        threading.Timer(0.5, self.complete_low_shot).start()

    def complete_low_shot(self):
        self.flywheel.spin_to_full_reverse_power()
        threading.Timer(1.5, self.finish_automatic_shot).start()


    def back_up_to_shoot(self):
        if self.shooter_timers_running:
            self.dt.set_dt_output(.4, .4)
            self.is_shooting = True
            threading.Timer(.7, self.finish_automatic_shot).start()
