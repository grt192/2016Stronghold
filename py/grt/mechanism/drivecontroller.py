...
class ArcadeDriveController:
    ...
    def __init__(self, dt, l_joystick, record_macro, operation_manager, xbox_controller, r_joystick=None):
        """
    ...
    def _joylistener(self, sensor, state_id, datum):
        if sensor in (self.l_joystick, self.r_joystick) and state_id in ('x_axis', 'y_axis'):
            if abs(datum) > .03:
                if self.manual_control_enabled:
                    power = self.xbox_controller.r_x_axis
                    turnval = self.xbox_controller.l_y_axis#self.r_joystick.x_axis if self.r_joystick else self.l_joystick.x_axis
                    # get turn value from r_joystick if it exists, else get it from l_joystick
                    self.dt.set_dt_output(power + turnval,
                                          power - turnval)
            else:
